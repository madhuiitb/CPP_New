...
public:
        ...
        for(auto i=0; i<threadCount;i++){
            m_vectorThreads.push_back(std::thread([this](){
                while(true){
                    ...
                    //   if(!m_count)
                            m_cv.wait(guard, [&](){return m_count>0;});
                        //m_chek=false;
                        m_count--;
                        work = std::move(wq.front());
                        wq.pop();
                    }
                        if(!work){
                        //    wq.push(work_item_ptr{nullptr});
                            // If no work will push another work item
                            // it will wake up one thread, until all the threads finishes the execution
                            wq.push(work_item_ptr{nullptr});
                         //  stop();
                           m_cv.notify_one();
                            break;
    ...
    */
    ...
    void do_work(work_item wt){
        auto do_work_item = std::make_unique<work_item>(std::move(wt));
        {
            std::unique_lock<std::mutex> ul(m_lock);
            wq.push(std::move(do_work_item));
            //m_chek = true;
            m_count++;
        }
        m_cv.notify_one(); // waiting many but waking up only thread
    }
    void stop(){
        {
            std::unique_lock<std::mutex> ul(m_lock);
...

        {
            std::unique_lock<std::mutex> ul(m_lock);
        //    if(m_stop) return;
            wq.push(std::move(work_item_ptr{nullptr}));
            //m_chek = true;
            m_count++;
            m_stop = true;
...
int main(){
    ...
    for(auto i=0; i<50;i++){
        tp.do_work([&, i=i](){
            unique_lock<mutex> ul_m(cout_guard);
